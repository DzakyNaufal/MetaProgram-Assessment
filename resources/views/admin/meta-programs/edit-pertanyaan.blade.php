...
@section('content')
    <div class="p-6">
        <div class="max-w-3xl mx-auto">
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-3xl">
                <div class="p-8">
                    <div class="mb-6">
                        <h1 class="text-2xl font-bold text-gray-900">Edit Pertanyaan Meta Program</h1>
                        <p class="text-gray-600 mt-1">Edit pertanyaan untuk Meta Program</p>
                    </div>

                    <form action="{{ route('admin.meta-programs.update-pertanyaan', $pertanyaan) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="meta_program_id" id="meta_program_id" required
                                    ...
                                    @foreach($metaPrograms as $mp)
                                        <option value="{{ $mp->id }}" {{ $pertanyaan->meta_program_id == $mp->id ? 'selected' : '' }}>{{ $mp->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea name="pertanyaan" id="pertanyaan" rows="3" required
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $pertanyaan->pertanyaan }}</textarea>
                                @error('pertanyaan')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-3 md:grid-cols-6 gap-3">
                                    <div>
                                        ...
                                        <input type="number" name="skala_sangat_setuju" value="{{ $pertanyaan->skala_sangat_setuju }}" min="1" max="6" required
                                            class="block w-full rounded border-gray-300 text-center">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Setuju (5)</label>
                                        <input type="number" name="skala_setuju" value="{{ $pertanyaan->skala_setuju }}" min="1" max="6" required
                                            class="block w-full rounded border-gray-300 text-center">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Agak Setuju (4)</label>
                                        <input type="number" name="skala_agak_setuju" value="{{ $pertanyaan->skala_agak_setuju }}" min="1" max="6" required
                                            class="block w-full rounded border-gray-300 text-center">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Agak Tidak (3)</label>
                                        <input type="number" name="skala_agak_tidak_setuju" value="{{ $pertanyaan->skala_agak_tidak_setuju }}" min="1" max="6" required
                                            class="block w-full rounded border-gray-300 text-center">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Tidak Setuju (2)</label>
                                        <input type="number" name="skala_tidak_setuju" value="{{ $pertanyaan->skala_tidak_setuju }}" min="1" max="6" required
                                            class="block w-full rounded border-gray-300 text-center">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Sangat Tidak (1)</label>
                                        <input type="number" name="skala_sangat_tidak_setuju" value="{{ $pertanyaan->skala_sangat_tidak_setuju }}" min="1" max="6" required
                                            class="block w-full rounded border-gray-300 text-center">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="keterangan" id="keterangan" rows="2"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $pertanyaan->keterangan }}</textarea>
                                @error('keterangan')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" name="is_negatif" id="is_negatif" {{ $pertanyaan->is_negatif ? 'checked' : '' }}
                                    class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                <label for="is_negatif" class="ml-2 block text-sm text-gray-900">Pertanyaan Negatif (skala dibalik)</label>
                            </div>

                            <div class="flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" {{ $pertanyaan->is_active ? 'checked' : '' }}
                                    class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        ...
                        <div class="mt-6 flex justify-end gap-3">
                            <a href="{{ route('admin.meta-programs.pertanyaan', $pertanyaan->meta_program_id) }}" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">
                                Batal
                            </a>
    ...
    <script>
        ...
        const selectedSubMetaProgramId = {{ $pertanyaan->sub_meta_program_id ?? 'null' }};
        ...
        function loadSubMetaPrograms(metaProgramId) {
            ...
            fetch(`/admin/meta-programs/sub-meta-programs/${metaProgramId}`)
                ...
                .then(data => {
                    subSelect.innerHTML = '<option value="">-- Pilih Sub Meta Program --</option>';
                    data.forEach(sub => {
                        const selected = sub.id === selectedSubMetaProgramId ? 'selected' : '';
        ...
        // Load on page load
        loadSubMetaPrograms({{ $pertanyaan->meta_program_id }});
    </script>
@endsection
