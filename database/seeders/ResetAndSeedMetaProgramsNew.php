...
class ResetAndSeedMetaProgramsNew extends Seeder
{
    /**
    ...
    {
        $this->command->info('Starting complete reset and reseed of Meta Programs...');
        ...
        $jsonPath = database_path('seeders/complete_data.json');

        if (file_exists($jsonPath)) {
            $questionsData = json_decode(file_get_contents($jsonPath), true);
            $this->importQuestionsFromJSON($questionsData, $course, $mpMap);
        } else {
            $this->command->warn('complete_data.json not found.');
        }
    ...
    private function importQuestionsFromJSON($data, $course, $mpMap)
    {
        $questionCount = 0;
        $skippedCount = 0;
